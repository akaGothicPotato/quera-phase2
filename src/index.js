...
function updateDoneTaskCount() {
  ...
  const formattedCount = doneTasksContainer.childElementCount
    .toString()
    .replace(/\d/g, (d) => "۰۱۲۳۴۵۶۷۸۹"[d]);
  doneTasksCount.textContent = `${formattedCount} تسک انجام شده است.`; // Update counter text
}
...
>
  ...
  <div class="max-md:py-3 max-md:px-4 md:px-5 md:py-6">
    ...
    <p
      id="description"
      class="description mr-9 text-Neutral-700 text-xs md:text-sm dark:text-task-desc-dark"
    >
      ${description}
    </p>
  </div>
...
function attachTaskEvents(taskElement) {
  ...
  checkbox.addEventListener("change", function () {
    ...
    const prioritySpan1 = taskElement.querySelector("#priority_1");
    const taskTitle = taskElement.querySelector("#Name");
    const descriptionParagraph = taskElement.querySelector(".description");

    if (this.checked) {
      taskTitle.classList.toggle("line-through");
      prioritySpan.classList.toggle("hidden");
      prioritySpan1.classList.toggle("hidden");
      descriptionParagraph.classList.toggle("hidden");
      doneTasksContainer.append(taskElement);
      updateTaskCount();
      updateDoneTaskCount();
    } else {
      taskTitle.classList.toggle("line-through");
...
createTaskBtn.addEventListener("click", () => {
  ...
  Artboard.classList.replace("flex", "hidden");
  submit.innerHTML = "اضافه کردن تسک";
});
